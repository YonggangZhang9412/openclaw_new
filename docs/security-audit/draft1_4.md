...
Our architecture addresses this governance gap by providing **verifiable security properties** that map directly to the specific requirements emerging in regulatory frameworks:

1. **Auditable decision trails** (addressing EU AI Act Article 12: Record-Keeping, and Article 14: Human Oversight). The Act requires that high-risk AI systems "be designed and developed in such a way to ensure that their operation is sufficiently transparent to enable deployers to interpret the system's output and use it appropriately." Every Gate decision produces a machine-readable log — tool name, arguments, token state, taint levels, Rule-of-Two evaluation, final decision with reason code — creating the "automatically generated logs" that Article 12 mandates. Unlike ambient authority models where the security rationale is "the LLM chose not to" (opaque), our audit trail provides deterministic, inspectable justification for every permitted and denied action.

2. **Deterministic security bounds** (addressing EU AI Act Article 9: Risk Management, and Article 15: Accuracy, Robustness and Cybersecurity). Article 15 requires that high-risk AI systems "be resilient as regards attempts by unauthorised third parties to alter their use, outputs or performance by exploiting the system vulnerabilities." The Rule-of-Two constraint provides a mathematically characterizable safety invariant — no execution path can simultaneously process untrusted input, access sensitive data, and perform external actions — that is verifiable through code inspection rather than statistical testing. This transforms cybersecurity compliance from a probabilistic claim ("we tested against known attacks") to a structural guarantee ("this class of attack chains is architecturally impossible within a single event batch").

3. **Per-event accountability** (addressing the accountability gap identified in the US Executive Order Section 4.1 and China's Interim Measures for Generative AI Article 7). Because permissions are scoped to individual event batches, each agent action can be traced to a specific triggering event, with its source, trust classification, and the capability token that authorized it. This creates an accountability chain connecting every autonomous action to a verifiable authorization decision — enabling regulators to audit not just *what* the agent did, but *why it was permitted to do so* at each step.

For the first time, it becomes possible to answer the regulator's question: "Can you prove that this agent cannot exfiltrate user data even if its language model is compromised?" Under ambient authority, the honest answer is "no." Under our architecture, the answer is: "Yes — the Rule-of-Two constraint structurally prevents the simultaneous presence of untrusted input, sensitive data access, and external action within a single event batch, and this constraint is enforced by deterministic code independent of the language model."
