...
**On the scope and permanence of this argument.** One might object: could a future architecture provide a *structural* type boundary between data and instructions — for instance, presenting external data in a separate API field with an explicit `type: "data"` tag, so the LLM receives `{instruction: "summarize this", data: "email content here"}` with an architectural separation?

This objection clarifies the precise scope of our argument. The Agent Authority Problem is an impossibility result **conditional on the dominant architecture of LLM-based agents** — systems where data and instructions share a processing channel. This includes all current LLM architectures (which concatenate inputs into a single token stream) and all foreseeable architectures that require the LLM to *reason about* external data (since reasoning requires the data to enter the model's computational context, where it can influence output generation). A hypothetical architecture with perfect channel separation — where external data is processed by a quarantined module that *never* shares state with the instruction-following module — would escape the impossibility. But such an architecture would itself be a form of the solution we propose: separating the security-critical processing path from the data-exposed processing path. CaMeL¹² takes a step in this direction with its Privileged/Quarantined LLM separation, but even CaMeL requires the Quarantined LLM to process untrusted data *and* generate tool-call decisions — meaning data and instructions still share a channel within the quarantined component. The conditionality does not diminish the result's practical significance: the architecture it applies to encompasses every deployed agent system as of 2026.
