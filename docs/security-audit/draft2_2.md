...
**Argument.** Under ambient authority, the agent holds privileges (C2) throughout the session. When processing untrusted data (C1), adversarial instructions embedded in that data enter the LLM's processing context. If data and instructions share a processing channel — as they do in all current LLM architectures, where both are concatenated into the same token stream — the LLM faces an inherently ambiguous classification task. To prevent the LLM from acting on adversarial instructions without reducing its privileges, the system must rely on the LLM's heuristic judgment to correctly classify content — a judgment that is probabilistic, context-dependent, and degradable under adversarial pressure. This violates C3.

**On the scope and permanence of this argument.** One might object: could a future architecture provide a *structural* type boundary between data and instructions — for instance, presenting external data in a separate API field with an explicit `type: "data"` tag, so the LLM receives `{instruction: "summarize this", data: "email content here"}` with an architectural (not linguistic) separation?

This objection is important and clarifies the precise scope of our argument. The Agent Authority Problem holds for **any system where data and instructions share a processing channel**. This includes all current LLM architectures (which concatenate inputs into a single token stream) and all foreseeable architectures that require the LLM to *reason about* external data (since reasoning requires the data to enter the model's computational context, where it can influence output generation).

A hypothetical architecture with perfect channel separation — where external data is processed by a quarantined module that *never* shares state with the instruction-following module — would indeed escape the impossibility. But such an architecture would be a form of the very solution we propose: separating the security-critical processing path from the data-exposed processing path. CaMeL¹² takes a step in this direction with its Privileged/Quarantined LLM separation, but even CaMeL requires the Quarantined LLM to process untrusted data *and* generate tool-call decisions — meaning data and instructions still share a channel within the quarantined component.

To be precise about scope: the Agent Authority Problem is an impossibility result **conditional on the dominant architecture of LLM-based agents** — systems where data and instructions share a processing channel. This includes all current LLM architectures and all foreseeable designs where the agent must *reason about* external data to act on it. A hypothetical future architecture achieving perfect channel separation would escape the impossibility, but would itself constitute a form of the architectural separation we advocate. The conditionality does not diminish the result's practical significance: the architecture it applies to encompasses every deployed agent system as of 2026.
