...
The permanence of the argument rests not on a property of natural language alone, but on a deeper architectural reality: **useful autonomous agents must reason about external data, and reasoning requires the data to enter the same computational context that generates actions**. As long as this holds, the data-instruction conflation is inherent, and the Agent Authority Problem applies.
