...
This vulnerability exists because LLMs process data and instructions through the same channel — the token stream. There is no hardware-enforced boundary between "this is data to analyze" and "this is an instruction to follow." The parallel to SQL injection is precise, instructive, and historically illuminating.

SQL injection became possible because early database interfaces concatenated user data and SQL commands into the same string. For over a decade, the prevailing response was **defensive programming**: input sanitization, escaping special characters, pattern-based detection of malicious queries. Each defense was incrementally bypassed through encoding tricks, comment injection, and second-order attacks. It took roughly 15 years — from the first documented SQL injection in 1998 to the widespread adoption of parameterized queries and prepared statements — for the field to recognize that the vulnerability was not in any specific code path but in the **architectural conflation of data and code**. The solution was not better sanitization but a structural separation: parameterized queries ensure that user input is *never interpreted as SQL commands*, regardless of its content.

Prompt injection stands at the beginning of the same evolutionary arc. The current state of the art — boundary markers, detection patterns, safety instructions, content wrapping — recapitulates the "sanitization era" of SQL security. These are necessary and valuable incremental defenses, but they cannot resolve the underlying conflation. **This paper proposes the "parameterized query moment" for agent security**: an architectural separation where the security enforcement layer (CapabilityGate) *never interprets natural language*, and the natural language processor (LLM) *never makes security decisions*. Just as parameterized queries removed the database engine from the business of parsing user input, our architecture removes the LLM from the business of enforcing security invariants.

But the consequences of failing to make this transition are far graver than in the SQL era. SQL injection could corrupt a database. Prompt injection of an autonomous agent can trigger real-world actions — sending emails, executing financial transactions, modifying medical records, deleting files — across every system the agent has access to. The blast radius is bounded only by the agent's permissions. And unlike SQL injection, where the attack required programming knowledge to craft malicious queries, prompt injection requires only natural language — lowering the attack barrier to anyone who can compose a sentence.
