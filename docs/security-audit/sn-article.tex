...
\paragraph{Trust-Level Zero Guarantee (informal).} For \textsc{Remote\_Open} events, the CapabilityIssuer grants no external-action tool. Since exfiltration requires transmitting data externally and no external-send tool is available, exfiltration is structurally impossible for this trust class---regardless of injection success, Rule-of-Two status, or taint evasion capability. This guarantee is unconditional: it depends only on the deterministic tool-grant policy of Table~\ref{tab:trust}. We formalize this as Corollary~\ref{cor:trust-zero} after introducing the probabilistic framework in Section~\ref{subsec:results}.
...
\paragraph{CVE-2026-25253: cross-site WebSocket hijacking RCE.} Under our architecture, an unauthenticated WebSocket message is classified as \textsc{Remote\_Open}. By the Trust-Level Zero Guarantee (Corollary~\ref{cor:trust-zero}), no external-action tool is granted, so $\Pr[\text{Exfil}] = 0$---the RCE chain is structurally impossible because code execution tools are never granted for untrusted sources.
...
\begin{theorem}[Residual-Risk Exfiltration Bound]\label{thm:main}
...
\end{equation}
where $p$ is the injection success probability, $kn_R/n$ and $kn_X/n$ are union bounds on the probability of sensitive-read and external-send tools being granted in the token (drawn jointly from the same token and therefore not probabilistically independent; the $\min$ reflects the tighter of the two union bounds rather than a product of independent barriers), $R_2$ is the Rule-of-Two indicator (zeroes the bound when enforced), and $q$ is the cross-batch taint evasion probability. Full proof in Supplementary Notes.
\end{theorem}

\begin{corollary}[Trust-Level Zero Guarantee]\label{cor:trust-zero}
For any event batch $b$ classified as \textsc{Remote\_Open}, the CapabilityIssuer grants no external-action tool, so $n_X^{\text{eff}}(b) = 0$ and hence $\alpha_X(b) = 0$. Applying Theorem~\ref{thm:main}:
\begin{equation}
  \Pr[\text{Exfil}(b) \mid b \in \textsc{Remote\_Open}] \leq p \cdot \min\!\left(\tfrac{kn_R}{n},\, 0\right) \cdot (1-R_2) \cdot q = 0.
\end{equation}
This guarantee is unconditional in $p$, $q$, and $R_2$; it depends only on the deterministic tool-grant policy of Table~\ref{tab:trust}.
\end{corollary}
...
Within a single batch, the causal taint sequence is monotonically non-decreasing ($\tau_i \leq \tau_j$ for $i \leq j$). Consequently, once \textsc{External} data is observed within a batch, within-batch taint evasion is exactly zero ($q_{\text{within}} = 0$). The statement is restricted to a single batch; cross-batch residuals are addressed separately in Section~\ref{subsec:crossbatch}.
\end{theorem}
...
\begin{itemize}
  ...
  \item Trust-level tool restriction: \textsc{Remote\_Open} events have $\alpha_X = 0$ (Corollary~\ref{cor:trust-zero}), yielding $\Pr[\text{Exfil}] = 0$ independent of the bound's other factors
...
\paragraph{Hierarchy of security guarantees.} The results above establish two qualitatively distinct classes of guarantee, and the architecture's practical security rests primarily on the first:

\emph{Deterministic (primary) guarantees.} Theorem~\ref{thm:r2} drives $\Pr[\text{Exfil}(b)]$ to exactly zero whenever $R_2 = 1$, and Corollary~\ref{cor:trust-zero} does the same whenever the batch is \textsc{Remote\_Open}. Both hold unconditionally in $p$ and $q$, and neither depends on any statistical assumption about the adversary---they follow from deterministic properties of the tool-grant policy and the Rule-of-Two check, each of which is enforced by LLM-independent code (Theorem~\ref{thm:integrity}).

\emph{Probabilistic (residual) bound.} Theorem~\ref{thm:main} characterizes residual risk in the regime where neither deterministic guarantee applies---for instance, a \textsc{Local\_Trusted} batch whose granted tools span $\mathcal{T}_U$, $\mathcal{T}_R$, and $\mathcal{T}_X$ because of a tool-classification error that defeats the Rule-of-Two check. In that degraded regime the bound records a single-factor tool-visibility reduction $\min(\alpha_R, \alpha_X)$ and an independent cross-batch taint-evasion factor $q$, rather than two independent multiplicative barriers. The bound should therefore be read as quantifying residual risk when the primary guarantees are bypassed or misconfigured, not as the main security property of the system.

The ``multi-barrier'' framing in the paper refers to the full architecture---EventInjectionGate, trust-level restriction, Rule of Two, token scoping, path scoping, and taint tracking---each of which operates on a distinct channel. Equation~\eqref{eq:main} aggregates only the CapabilityGate-internal portion of these barriers that admits a closed-form probabilistic analysis.
...
Our dual-gate event-driven capability architecture demonstrates that this problem can be circumvented by replacing the authorization model. The EventInjectionGate constrains what enters the system; the CapabilityGate constrains what the LLM can do. Together they transform agent security from a single-barrier model (dependent on LLM injection resistance) to a layered architecture whose primary security rests on two deterministic zero-probability guarantees---for untrusted sources (Corollary~\ref{cor:trust-zero}) and for Rule-of-Two-enforced batches (Theorem~\ref{thm:r2})---with a probabilistic residual-risk bound (Theorem~\ref{thm:main}) characterizing the degraded regime where neither deterministic guarantee applies.
